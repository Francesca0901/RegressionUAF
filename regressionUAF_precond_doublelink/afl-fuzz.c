...
static u8 *seed_prob_norm_buf,                  /* normed probability of seeds */
...
EXP_ST u8 skip_deterministic, /* Skip deterministic stages?       */
    ...
    precondmap_changed = 1,   // SPUPDATE 4 // not used so far
...
EXP_ST u8 *trace_precond_bits; // SHM for precondition coverage // SPUPDATE 1
static s32
    shm_precond_id; // ID for the SHM region for precondition // SPUPDATE 1

EXP_ST u8 virgin_bits[MAP_SIZE],  /* Regions yet untouched by fuzzing */
    virgin_tmout[MAP_SIZE],       /* Bits we haven't seen in tmouts   */
    virgin_crash[MAP_SIZE],       /* Bits we haven't seen in crashes  */
    virgin_precond[MAP_SIZE]; // SPUPDATE 1
...
static u64 total_bitmap_size, /* Total bit count for all bitmaps  */
    ...
    // SPUPDATE 4
    total_precondmap_size,   /* Total bit count for all precondmaps  */
    total_precondmap_entries;  /* Number of precondmaps counted        */
...
struct queue_entry {
  ...
  u32 bitmap_size, /* Number of bits set in bitmap     */
      precondmap_size, // to calculate numbers of bits set in precond map // SPUPDATE 4 
  ...
  u64 exec_us,  /* Execution time (us)              */
      ...
      depth;    /* Path depth                       */
  double raw_fitness,         /* The non-normalized fitness of the seed as it is returned */
  ...
  u32 tc_ref;     /* Trace bytes ref count            */ 
      //how many times this testcase is selected in top_rate[]
...
static inline void update_byte_score_havoc(struct queue_entry* q, double cur_fitness,
  ...
  } else if(aco_incdec == ACO_INC_DEC && cur_fitness + delt < q->weight){
...
static inline void update_byte_score_deterministic(struct queue_entry* q, double cur_fitness, 
  ...
  if (group_start_pos == group_end_pos){
    ...
    // the function only updates the scores for the groups at the beginning and end positions. 
    if (group_start_pos < group_max_pos)
        update_byte_score_havoc(q, cur_fitness, group_byte_score + group_start_pos);
...
void create_byte_alias_table(struct queue_entry* q){
  ...
  if (!byte_prob_norm_buf) {
    byte_prob_norm_buf = (u8 *)ck_alloc(n * sizeof(double));
    byte_out_scratch_buf = (u8 *)ck_alloc(n * sizeof(int));
    byte_in_scratch_buf = (u8 *)ck_alloc(n * sizeof(int));
    aco_max_seed_len = n;
  } else if (aco_max_seed_len < n){
...
void create_seed_alias_table(void){
  ...
  if (!seed_alias_table){
    seed_alias_table = (u32 *)ck_alloc(n * sizeof(u32));
    seed_alias_probability = (double *)ck_alloc(n * sizeof(double));

    seed_prob_norm_buf = (u8 *)ck_alloc(n * sizeof(double));
    seed_out_scratch_buf = (u8 *)ck_alloc(n * sizeof(int));
    seed_in_scratch_buf = (u8 *)ck_alloc(n * sizeof(int));
  } else{
    ...
    seed_out_scratch_buf = (u8 *)ck_realloc((void *)seed_out_scratch_buf, n * sizeof(int));
    seed_in_scratch_buf = (u8 *)ck_realloc((void *)seed_in_scratch_buf, n * sizeof(int));
  }

  double *P = (double *)seed_prob_norm_buf;
  int *   S = (int *)seed_out_scratch_buf;
  int *   L = (int *)seed_in_scratch_buf;
...
// SPUPDATE 4
// virgin_precond_map should be virgin_precond, not nessarily to be passed by argument
static inline u8 has_new_precond_bits(u8* virgin_precond_map) {

#ifdef WORD_SIZE_64

  u64* current = (u64*)trace_precond_bits;
  u64* virgin  = (u64*)virgin_precond_map;

  u32  i = (MAP_SIZE >> 3); // how many 64-bit chunks in the map

#else

  u32* current = (u32*)trace_precond_bits;
  u32* virgin  = (u32*)virgin_precond_map;

  u32  i = (MAP_SIZE >> 2); // how many 32-bit chunks in the map
...
// has any bits set?    are they also set in virgin?
    if (unlikely(*current) && unlikely(*current & *virgin)) {

      if (likely(ret < 2)) { // if we haven't found entire new paths yet
...
#ifdef WORD_SIZE_64
        // If any byte in the current trace is non-zero 
        // and its corresponding byte in the virgin map is 0xff (i.e., it's a completely new path)
        // set ret to 2.
        if ((cur[0] && vir[0] == 0xff) || (cur[1] && vir[1] == 0xff) ||
            (cur[2] && vir[2] == 0xff) || (cur[3] && vir[3] == 0xff) ||
            (cur[4] && vir[4] == 0xff) || (cur[5] && vir[5] == 0xff) ||
  ...
  if (ret) precondmap_changed = 1;

  // 0 if no new paths or branches were found.
  // 1 if new branches but not new paths were found.
  // 2 if entirely new paths were discovered.
  return ret;
...
//   It uses shmctl with the IPC_RMID command to mark the shared memory segment
//   for deletion. Once marked in this way, the segment will actually be deleted
//   when no process is attached to it.
static void remove_shm(void) {

  shmctl(shm_id, IPC_RMID, NULL);
  shmctl(shm_precond_id, IPC_RMID, NULL); // SPUPDATE 1
}
...
static void minimize_bits(u8* dst, u8* src) {
 ...
 // i >> 3 finds the index of the byte in the dst array that contains the bit
  // we want to set. 1 << (i & 7) finds position of the set bit (equivalent to i
  // / 8), which gives the position within the byte of the bit we want to set.
  // dst[i >> 3] |= ... sets the bit in the dst byte without changing the other
  // bits in the byte.
  while (i < MAP_SIZE) {
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  u32 i;
  u64 fav_factor = q->exec_us * q->len; // the product of the execution time and
                                        // the length of the test case
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) { //  If there is already a winning test case
                          //  (top_rated[i]) for this byte of the bitmap
...
static void cull_queue(void) {
  ...
  memset(temp_v, 255, MAP_SIZE >> 3); // temp_v : 0xff 0xff 0xff 0xff ...

  //  Resets the counters for favored queue entries (test cases that are
  ...
  //  goes through each entry in the queue and resets its "favored" status.
  while (q) {
    q->favored = 0;
    q = q->next;
  }
  ...
  for (i = 0; i < MAP_SIZE; i++)
        if (top_rated[i] &&
        (temp_v[i >> 3] &
         (1 << (i & 7)))) { // If a top_rated entry exists for a byte and that
                            // byte's corresponding bit is set in temp_v, the
                            // function then:
      ...
      //  ensuring that temp_v only retains the bits not yet covered by favored
      //  entries.
      while (j--) 
      ...
      if (!top_rated[i]->was_fuzzed) // If the top_rated entry has not been
                                     // fuzzed yet, also increments the count of
                                     // pending favored entries
        pending_favored++;
    }
  ...
  while (q) {
    mark_as_redundant(
      q,
      !q->favored); // marking as redundant all entries that are not favored.
    q = q->next;
  }
...
EXP_ST void setup_shm(void) {
  ...
  memset(virgin_precond, 255, MAP_SIZE); // SPUPDATE 1
 ...
 //------- initialize precondition_bits // SPUPDATE 1
  shm_precond_id =
      shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_precond_id < 0)
    PFATAL("shmget() for precondition_map failed");
  //------
  atexit(remove_shm);

  shm_str = alloc_printf("%d", shm_id);
  shm_precond_str = alloc_printf("%d", shm_precond_id); // SPUPDATE 1
...
if (!dumb_mode) {
  ...
  setenv(PRECOND_SHM_ENV_VAR, shm_precond_str, 1); //SPUPDATE 1
}

  ck_free(shm_str);
  ck_free(shm_precond_str); // SPUPDATE 1


  trace_bits = shmat(shm_id, NULL, 0);
  trace_precond_bits = shmat(shm_precond_id, NULL, 0); // SPUPDATE 1

  
  if (trace_bits == (void *)-1) PFATAL("shmat() failed");

  if (trace_precond_bits == (void *)-1) // SPUPDATE 1
    PFATAL("shmat() for precondition failed");
}
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  memset(trace_precond_bits, 0, MAP_SIZE); //SPUPDATE 4
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  if (out_file) {
    // try to delete out_file
    unlink(out_file); /* Ignore errors. */

    // O_EXCL means it won't overwrite it, 0600 means read and write owner only
  ...
  if (!out_file) {
    ...
    lseek(fd, 0, SEEK_SET); //Again, it sets the offset of the file descriptor back to the beginning.
...
static void write_with_gap(void* mem, u32 len, u32 skip_at, u32 skip_len) {
  ...
  // skip_at: the starting index of the gap, skip_len: the length of the gap
  ...
  // the gap is not at the very beginning of the data, it writes the part of
  // the data before the gap
  if (skip_at) ck_write(fd, mem, skip_at, out_file);

  // the gap is not at the very end of the data, it writes the part of
  // the data after the gap
...
// meaning it runs the target program with the test case as input, monitors the
// execution, and collects data that is used later for analysis
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  u8 fault = 0, new_bits = 0, var_detected = 0, hnb = 0, 
      hnprecond = 0, new_precond_bits = 0, // SPUPDATE 4
  ...
  // copying the trace_bits (result of the instrumentation of a test run) into
  // first_trace if a checksum already exists for the test case, then checks if
  // the new execution path (defined by the trace_bits) includes new bits that
  // were not covered
  if (q->exec_cksum) {
    ...
    // SPUPDATE 4
    hnprecond = has_new_precond_bits(virgin_precond); //do this because this also sync virgin_precond with trace_precond_bits
    if (hnprecond > new_precond_bits)
      new_precond_bits = hnprecond;
  }

  start_us = get_cur_time_us();

  // The test case is then run stage_max times
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    // It then checks if the newly calculated checksum cksum is different
    // from the stored checksum q->exec_cksum. If they are different, this means
    // that the same test case has resulted in a different execution path.
    if (q->exec_cksum != cksum) {

      hnb = has_new_bits(virgin_bits); // 0 or 1 or 2
      if (hnb > new_bits)
        new_bits = hnb;
      ...
      if (q->exec_cksum) {
        ...
        for (i = 0; i < MAP_SIZE; i++) {
          // compare the first_trace array and current trace_bits to detect if
          // there are any varying (fluctuating) bytes. If such bytes exist, it
          // marks them in var_bytes array and it also increases the calibration
          // cycles stage_max = CAL_CYCLES_LONG.
  ...
  q->handicap    = handicap;
  q->cal_failed  = 0;
  q->precondmap_size = count_bytes(trace_precond_bits); // SPUPDATE 4
  ...
  // SPUPDATE 4
  if (q->precondmap_size > 0) {
    total_precondmap_size += q->precondmap_size;
    total_precondmap_entries++;
  }
  ...
  if (!dumb_mode && first_run && !fault && !new_bits && !new_precond_bits) // SPUPDATE 4
    fault = FAULT_NOBITS;
...
// test the first testcase trigger enough covs
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    // It reads the test case data into memory.
    ...
    switch (res) {
      ...
      case FAULT_NONE:
        ...
        if (q == queue) // check the only input enough or not
          check_map_coverage();
      ...
      case FAULT_CRASH:  
      // If crash_mode is set (i.e., the fuzzer is specifically looking for
      // crashes), AFL continues with the next test case.
        if (crash_mode)
          break;
  ...
  OKF ("Initial Churn Raw Fitness is: %.6f.", (max_raw_fitness + min_raw_fitness)/2);
...
// When you create a hard link, you are essentially creating a new name for
// existing data.
static void link_or_copy(u8* old_path, u8* new_path) {
  ...
  if (!i)
   return;
  // if can't link (might coz src and tar in different filesys)
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    // SPUPODATE 4 ：also check if there are new bits in the preconditionmap
    if (!(hnb = has_new_bits(virgin_bits)) && !(hnprecond = has_new_precond_bits(virgin_precond))) { 
      if (crash_mode)
        total_crashes++;
      return 0;
    }    
  ...
  // when crash_mode is not set, the fuzzer might be looking for other types of
  // interesting behavior, such as new code coverage or program hangs
  switch (fault) {
    ...
    case FAULT_TMOUT:
      ...
      if (!dumb_mode) {
        ...
        if (!has_new_bits(virgin_tmout)  && !has_new_precond_bits(virgin_precond)) // SPUPDATE 4
          return keeping;
      }
...
keep_as_crash:
      ...
      if (!dumb_mode) {
        ...
        if (!has_new_bits(virgin_crash) && !has_new_precond_bits(virgin_precond)) // SPUPDATE 4
          return keeping;
      }

      if (!unique_crashes) // if this is first crash
        write_crash_readme();
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  // bitmap_cvg: cov achieved so far
  // stability: how consistently the target program behaves
  ...
  fprintf(f, "start_time        : %llu\n"
             ...
             orig_cmdline, slowest_exec_ms, total_precondmap_entries, total_bitmap_entries);
             /* ignore errors */
...
static void show_stats(void) {
  ...
  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
          ...
          cYEL "aflchurn & Erchang", use_banner);
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  // SPUPDATE 4 & 5
  u32 avg_precondmap_size = total_precondmap_entries > 0 ? total_precondmap_size / total_precondmap_entries : 0;
  u32 perf_score = 100;
  ...
  // SPUPDATE 4 & 5
  // Questions!
  // Adjust score based on precondition map size. I still didn't decide the Multiplier
  ...
  // SPUPDATE 6: very straightforward score update
  // if(q->precondmap_size == 0) perf_score *= 0.25;
  // else if(q->precondmap_size > avg_precondmap_size) perf_score *= 2;
  // else if(q->precondmap_size < avg_precondmap_size) perf_score *= 0.5;
  ...
  // depth: the number of mutations that have been applied to reach the current
  // state from the original seed input
  switch (q->depth) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
        ...
        case 0:
          ...
          FLIP_BIT(out_buf, URfitness(queue_cur, temp_len) * 8 + UR(8));
        ...
        case 1: 
          ...
          out_buf[URfitness(queue_cur, temp_len)] = 
        ...
        case 2:
          ...
          if (UR(2)) {
            ...
            *(u16*)(out_buf + URfitness(queue_cur, temp_len - 1)) =
              interesting_16[UR(sizeof(interesting_16) >> 1)];

          } else {
            
            *(u16*)(out_buf + URfitness(queue_cur, temp_len - 1)) = SWAP16(
              interesting_16[UR(sizeof(interesting_16) >> 1)]);
        ...
        case 3:
          ...
          if (UR(2)) {
            ...
            *(u32*)(out_buf + URfitness(queue_cur, temp_len - 3)) =
              interesting_32[UR(sizeof(interesting_32) >> 2)];

          } else {
            
            *(u32*)(out_buf + URfitness(queue_cur, temp_len - 3)) = SWAP32(
              interesting_32[UR(sizeof(interesting_32) >> 2)]);
        ...
        case 4:
          ...
          out_buf[URfitness(queue_cur, temp_len)] -= 1 + UR(ARITH_MAX);
          break;
        ...
        case 6:
          ...
          if (UR(2)) {
            ...
            u32 pos = URfitness(queue_cur, temp_len - 1);
        ...
        case 8:
          ...
          if (UR(2)) {
            ...
            u32 pos = URfitness(queue_cur, temp_len - 3);
        ...
        case 10:
          ...
          out_buf[URfitness(queue_cur, temp_len)] ^= 1 + UR(255);
          break;
        ...
        case 11 ... 12: {
            ...
            del_from = URfitness(queue_cur, temp_len - del_len + 1);
        ...
        case 13:
          ...
          if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
            ...
            clone_to   = URfitness(queue_cur, temp_len);
        ...
        case 14: {
            ...
            copy_to   = URfitness(queue_cur, temp_len - copy_len + 1);
        ...
        case 15: {
            ...
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
              ...
              insert_at = URfitness(queue_cur, temp_len - extra_len + 1);
        ...
        case 16: {
            ...
            /* If not inserting at the end, use byte score to select insert location */
            if (insert_at != temp_len) 
                insert_at = URfitness(queue_cur, temp_len);
...
void plot_precondmap(){
  u8* tmp;
  s32 fd;
  FILE* precond_file;
  struct queue_entry *q = queue, *n;

  tmp = alloc_printf("%s/precondmap", out_dir);
  fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (fd < 0) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  precond_file = fdopen(fd, "w");
  if (!precond_file) PFATAL("fdopen() failed");

  while (q) {
    n = q->next;
    if (q->precondmap_size)
      fprintf(precond_file, "%d\n", q->precondmap_size);
    q = n;
  }
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "afl-fuzz " cBRI VERSION cRST " by <lcamtuf@google.com> & Erchang\n");
...
stop_fuzzing:
  ...
  // plot precondition map bytes counts
  // plot_byte_score();
  plot_precondmap();

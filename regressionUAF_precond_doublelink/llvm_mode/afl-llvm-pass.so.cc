...
// check if big_str starts with small_str
bool startsWith(std::string big_str, std::string small_str) {
  if (big_str.compare(0, small_str.length(), small_str) == 0)
    return true;
  else
    return false;
}

// calculates a normalized age
double inst_norm_age(int max_days, int days_since_last_change) {
  double norm_days;
  // if (days_since_last_change < 0) norm_days = 1;
  // else norm_days =
  //     1 / (log2(days_since_last_change + 2) * log2(days_since_last_change +
  //     2));
...
// a lower rank will result in a higher score
double inst_norm_rank(unsigned int max_rank, int line_rank) {
  ...
  if (line_rank <= 0) {
    ...
    norm_ranks = 1 / (double)line_rank;

  return norm_ranks;
}

double inst_norm_change(unsigned int num_changes,
                        unsigned short change_select) {
  double norm_chg = 0;
...
std::string execute_git_cmd(std::string directory, std::string str_cmd) {
  ...
  fp = popen(git_cmd.str().c_str(), "r"); //excute git_cmd, fp is to read the output
  if (NULL == fp)
    return str_res;
  // when cmd fail, output "fatal: ...";
  // when succeed, output result
  if (fscanf(fp, "%s", ch_git_res) == 1) {
    str_res.assign(ch_git_res); //if read the output of the command successful, the content is assigned to the str_res string
  }
...
int get_threshold_changes(std::string directory) {
  ...
  std::ostringstream changecmd; // build the cmd string
  unsigned int largest_changes = 0;
  int change_threshold = 0;
  FILE *dfp;
  // The maximum number of changes to any file.
  changecmd << "cd " << directory
            << " && git log --name-only --pretty=\"format:\"" // get changes log
            << " | sed '/^\\s*$/d' | sort | uniq -c | sort -n"
            << " | tr -s ' ' | sed \"s/^ //g\" | cut -d\" \" -f1 | tail -n1";
  // execute cmd and returns a file pointer to read its output
  dfp = popen(changecmd.str().c_str(), "r");
  if (NULL == dfp)
    return WRONG_VALUE;

  if (fscanf(dfp, "%u", &largest_changes) != 1)
    return WRONG_VALUE;

  // change threshold: 1/10 of largest changes
...
unsigned long get_commit_time_days(std::string directory, std::string git_cmd) {
  ...
  return unix_time / 86400; //86400 is the number of second in a day -> calculate days
}
...
unsigned int get_max_ranks(std::string git_directory) {
  ...
  headcmd << "cd " << git_directory << " && git rev-list --count HEAD";
...
/*
 ...
 Help get the related lines in HEAD commits, which are related to the lines from
 git show.
 */
void git_diff_current_head(
    std::string cur_commit_sha, std::string git_directory,
    std::string relative_file_path,
    std::set<unsigned int> &changed_lines_from_show, // a set of changed lines
    std::map<unsigned int, unsigned int> &lines2changes) { // a map to track

  std::ostringstream cmd;

  // char array to store parsed results from "git diff"
  ...
  int rc = 0; // store return code of pclose()
  FILE *fp;
  int cur_line_num, cur_num_start, cur_num_count, head_line_num, head_num_start,
      head_num_count;
  ...
  */
  cmd << "cd " << git_directory << " && git diff -U0 " << cur_commit_sha
  ...
  /* -: current_commit;
     +: HEAD */
  // result: "@@ -8,0 +9,2 @@" or "@@ -10 +11,0 @@" or "@@ -466,8 +475 @@" or
  // "@@ -8 +9 @@"
  //@@ -8,0 +9,2 @@: In the original file, starting from line 8, 0 lines are affected. In the new file, starting from line 9, 2 lines have been added.
  //@@ -466,8 +475 @@: In the original file, starting from line 466, 8 lines are affected. In the new file, line 475 is affected. Since there's no comma and second number, it means only this one line is affected.
  while (fscanf(fp, "%s %s %s %s", fatat, array_current_changes,
                array_head_changes, tatat) == 4) {
    ...
    /* If the changed lines in current commit can be found in
       changed_lines_from_show, the related lines in HEAD commit should count
       for changes. */
    if (cur_comma_pos == std::string::npos) { // only one line changed
      cur_line_num = std::stoi(current_line_range);
      if (changed_lines_from_show.count(cur_line_num))
        is_head_changed = true;
    } else { // multiple lines changed
      cur_num_start = std::stoi(current_line_range.substr(0, cur_comma_pos));
      cur_num_count = std::stoi(current_line_range.substr(
          cur_comma_pos + 1, current_line_range.length() - cur_comma_pos - 1));
    ...
    /* Trace changes for head commit, increment lines2changes.
      Some lines are changed in current commit, so trace these lines back to
      HEAD commit, and increment the count of these lines in HEAD commit.
      */
    if (is_head_changed) {
      ...
      if (head_comma_pos == std::string::npos) {
        ...
        head_num_count = std::stoi(head_line_result.substr(
            head_comma_pos + 1,
  ...
  /* if there's no diff in current commit and HEAD commit;
   ...
   so any change in current commit (compared to its parents) counts for the HEAD
   commit*/

  if (!cur_head_has_diff) {
    for (auto mit = changed_lines_from_show.begin();
         mit != changed_lines_from_show.end(); ++mit) {
      if (lines2changes.count(*mit))
        lines2changes[*mit]++;
      else
        lines2changes[*mit] = 1;
    }
...
/* git show, get changed lines in current commit.
    It'll show you the log message for the commit, and the diff of that
   particular commit. Find the changed line numbers in file relative_file_path
   as it was changed in commit cur_commit_sha, and add them to the list
   changed_lines_cur_commit
 */
void git_show_current_changes(
    std::string cur_commit_sha, std::string git_directory,
    std::string relative_file_path,
  ...
  char array_parent_changes[32], array_current_changes[32], fatat[12],
      tatat[12];
  ...
  // result: "@@ -8,0 +9,2 @@" or "@@ -10 +11,0 @@" or "@@ -466,8 +475 @@" or
  // "@@ -8 +9 @@"
  cmd << "cd " << git_directory << " && git show --oneline -U0 "
      << cur_commit_sha << " -- " << relative_file_path
      << " | grep -o -P \"^@@ -[0-9]+(,[0-9])? \\+[0-9]+(,[0-9])? @@\"";
  ...
  while (fscanf(fp, "%s %s %s %s", fatat, array_parent_changes,
                array_current_changes, tatat) == 4) {
    ...
    if (comma_pos == std::string::npos) {
      ...
      if (line_num >= 0)
        changed_lines_cur_commit.insert(line_num);
    } else {
      num_start = std::stoi(current_line_range.substr(0, comma_pos));
      num_count = std::stoi(current_line_range.substr(
          comma_pos + 1, current_line_range.length() - comma_pos - 1));
...
*/
bool calculate_line_age_git_cmd(
    std::string relative_file_path, std::string git_directory,
  ...
  */
  ...
  if (head_commit_days == WRONG_VALUE || init_commit_days == WRONG_VALUE)
    return false;

  int max_days = head_commit_days - init_commit_days;

  cmd << "cd " << git_directory << " && git blame --date=unix "
      << relative_file_path << " | grep -o -P \"[0-9]{9}[0-9]? +[0-9]+\"";
...
/* get rank of line ages.
  rank = (the number of commits until HEAD) - (the number of commits until
  commit A);
 */
bool cal_line_age_rank(
    std::string relative_file_path, std::string git_directory,
    std::map<std::string, std::map<unsigned int, double>> &file2line2rank_map,
    std::map<std::string, double> &commit2rank, unsigned int head_num_parents) {
  ...
  if (head_num_parents == WRONG_VALUE)
    return false;
  ...
  blamecmd << "cd " << git_directory << " && git blame -p -- "
           << relative_file_path << " | grep -o \"^[0-9a-f]* [0-9]* [0-9]*\"";
  ...
  while (fscanf(dfp, "%s %u %u", line_commit_sha, &nothing_line, &line_num) ==
         3) {
    ...
    if (commit2rank.count(str_cmt)) {
      ...
      rankcmd.str("");
      rankcmd.clear();
      rankcmd << "cd " << git_directory << " && git rev-list --count "
              << line_commit_sha;
  ...
  if (!line_rank.empty())
    file2line2rank_map[relative_file_path] = line_rank;
  return true;
}
...
bool is_file_exist(std::string relative_file_path, std::string git_directory) {
  ...
  // string cmd("cd /home/usrname/repo && git cat-file -e HEAD:util/read.h
  // 2>&1");
  std::ostringstream cmd;
  ...
  if (access(git_directory.c_str(), F_OK) == -1)
    return false;

  cmd << "cd " << git_directory
      << " && git cat-file -e HEAD:" << relative_file_path << " 2>&1";
...
/* Change the filename to relative path (relative to souce dir) without "../" or
"./" in the path. Input: relative_file_path: relative path of source files,
relative to base_directory base_directory: absolute path of directories in
building directory git_directory: absolute path of git repo directory (root of
source code) Output: clean relative path of a file
 */
std::string get_file_path_relative_to_git_dir(std::string relative_file_path,
  ...
  if (startsWith(relative_file_path, "/")) {
    ...
    // remove substring, which is the same as git_directory, from
    // relative_file_path
    ...
    // relative_file_path could be src/file.c, build/../src/file.c, or
    // src/./file.c relative_file_path is relative to base_directory here
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  if (isatty(2) && !getenv("AFL_QUIET")) {
    ...
    SAYF(cCYA "afl-llvm-pass " cBRI VERSION cRST " by <aflchurn & Erchang>\n");
  ...
  if (getenv("AFLCHURN_DISABLE_AGE"))
    use_cmd_age = false;
  ...
  if (getenv("AFLCHURN_DISABLE_CHURN"))
    use_cmd_change = false;
  change_sig_str = getenv("AFLCHURN_CHURN_SIG"); //decide churn signal to use
  if (change_sig_str) {
    if (!use_cmd_change)
      FATAL("Cannot simultaneously set AFLCHURN_DISABLE_CHURN and "
            "AFLCHURN_CHURN_SIG!");
  ...
  if (bb_select_ratio_str) {
    ...
    if (sscanf(bb_select_ratio_str, "%u", &bb_select_ratio) != 1 ||
        !bb_select_ratio || bb_select_ratio > 100)
  ...
  int inst_free_blocks = 0; // SPUPDATE 2

  int inst_blocks = 0, inst_ages = 0, inst_changes = 0, inst_fitness = 0;
  double module_total_ages = 0, module_total_changes = 0,
         module_total_fitness = 0, module_ave_ages = 0, module_ave_chanegs = 0,
         module_ave_fitness = 0;
  ...
  std::map<std::string, std::map<unsigned int, double>> map_age_scores,
      map_bursts_scores, map_rank_age;
  ...
  for (auto &F : M) {
    ...
    if (git_no_found && !is_one_commit) {
      ...
      F.getAllMetadata(MDs);// extracts all metadata from a function (F) in the module and stores it in MDs
      for (auto &MD : MDs) {
        if (MDNode *N = MD.second) {
          if (auto *subProgram = dyn_cast<DISubprogram>(N)) {
            ...
            if (!funcfile.empty() && !funcdir.empty()) {
              // fix path here; if "funcfile" does not start with "/", use
              // funcdir as the prefix of funcfile
              if (!startsWith(funcfile, "/")) {
                func_abs_path = funcdir;
                func_abs_path.append("/");
                func_abs_path.append(funcfile);
              } else
                func_abs_path = funcfile;
              ...
              if (!git_no_found) {
                ...
                func_abs_path = func_abs_path.substr(
                    0, func_abs_path.find_last_of(
                           "\\/")); // remove filename in string
                // git rev-parse --show-toplevel: show the root folder of a
                // repository
                ...
                if (git_path.empty())
                  ...
                  git_path.append("/"); // result: /home/usr/repo_name/
                ...
                if (!git_no_found) {
                  ...
                  if (commit_cnt.compare("1") == 0) { // only one commit
                    git_no_found = 1;
                    is_one_commit = 1;
                    OKF("Shallow repository clone. Ignoring file %s.",
                        funcfile.c_str());
                    break;
                  }
                  // #change threshold
                  ...
                  std::string init_cmd("git log --reverse --date=unix "
                                       "--oneline --format=%cd | head -n1");
                  init_commit_days = get_commit_time_days(git_path, init_cmd);
                  /* Get the number of commits before HEAD */
                  head_num_parents = get_max_ranks(git_path);
                  /* thresholds */
                  norm_change_thd =
                      inst_norm_change(THRESHOLD_CHANGES, change_sig);
                  norm_age_thd = inst_norm_age(
                      head_commit_days - init_commit_days, THRESHOLD_DAYS);
                  norm_rank_thd =
                      inst_norm_rank(head_num_parents, THRESHOLD_RANKS);
                  break;
                }
              }
            }
    ...
    for (auto &BB : F) {
      ...
      // SPUPDATE 1 instrument free()
      int free_exist = 0;
      for (auto &I : BB) {
        if (CallInst *callInst = dyn_cast<CallInst>(&I)) {
          Function *callee = callInst->getCalledFunction();
          if (callee && callee->getName().contains("free")) {
            errs() << "Identified a free call in function " << F.getName()
                   << "\n";
            free_exist = 1;
          }
        }
      ...
      double bb_rank_age = 0, bb_age_best = 0, bb_burst_best = 0,
             bb_rank_best = 0;
      double bb_raw_fitness, tmp_score;
      bool bb_raw_fitness_flag = false;
      bool bb_changed = false; // SPUPDATE 3
      ...
      for (auto &I : BB) {
        ...
        if (Loc && !git_no_found) {
          ...
          if (!filename.empty() && !filedir.empty()) {
            ...
            clean_relative_path =
                get_file_path_relative_to_git_dir(filename, filedir, git_path);

            if (!clean_relative_path.empty()) {
              /* calculate score of a block */
              /* Check if file exists in HEAD using command mode */
              if (unexist_files.count(clean_relative_path))
                break;
              ...
              if (!bb_lines.count(line)) {
                ...
                if (!processed_files.count(clean_relative_path)) {
                  ...
                  /* the ages for lines */
                  if (use_cmd_age)
                    calculate_line_age_git_cmd(clean_relative_path, git_path,
                                               map_age_scores, head_commit_days,
                                               init_commit_days);
                  if (use_cmd_age_rank)
                    cal_line_age_rank(clean_relative_path, git_path,
                                      map_rank_age, commit_rank,
                                      head_num_parents);
                ...
                if (use_cmd_age) {
                  ...
                  if (map_age_scores.count(clean_relative_path)) {
                    if (map_age_scores[clean_relative_path].count(line)) {
                      ...
                      if (bb_age_best < tmp_score)
                        bb_rank_age = bb_age_best = tmp_score;
                    }
                  }
                }
                ...
                if (use_cmd_age_rank) {
                  if (map_rank_age.count(clean_relative_path)) {
                    if (map_rank_age[clean_relative_path].count(line)) {
                      ...
                      if (bb_rank_best < tmp_score)
                        bb_rank_age = bb_rank_best = tmp_score;
                    }
                  }
                }
                ...
                if (use_cmd_change) {
                  ...
                  if (map_bursts_scores.count(clean_relative_path)) {
                    if (map_bursts_scores[clean_relative_path].count(line)) {
                      ...
                      if (bb_burst_best < tmp_score)
                        bb_burst_best = tmp_score;
                      if (bb_burst_best > 0)
                        bb_changed = true;
                    }
                  }
      ...
      IRB.CreateStore(Incr, MapPtrIdx)->setMetadata(NoSanMetaId, NoneMetaNode);
      ...
      // SPUPDATE 3
      FunctionCallee updateBBListCallee = M.getOrInsertFunction(
          "update_bb_list", FunctionType::getVoidTy(C), Int32Ty);  // Assuming the function type matches
      Function *updateBBListFunc = cast<Function>(updateBBListCallee.getCallee());

      FunctionCallee updateFreeListCallee = M.getOrInsertFunction(
          "update_free_list", FunctionType::getVoidTy(C), Int32Ty);  // Assuming the function type matches
      Function *updateFreeListFunc = cast<Function>(updateFreeListCallee.getCallee());
      ...
      if ((use_cmd_age || use_cmd_age_rank) && !use_cmd_change) {
        ...
        if ((bb_burst_best > 0) && // only when change is assigned
          ...
          // // SPUPDATE 3
          // errs() << "Identified a patched block in function " << F.getName() << "\n";
          // errs() << "try to call updateBBListFunc\n";
          // if (updateBBListFunc) {
          //     IRB.CreateCall(updateBBListFunc, { CurLoc });
          //     errs() << "call updateBBListFunc\n";
          // }
      ...
      if (free_exist) {
          errs() << "try to call updateFreeListFunc\n";
          inst_free_blocks++;
          if (updateFreeListFunc) {
              IRB.CreateCall(updateFreeListFunc, { CurLoc });
              errs() << "call updateFreeListFunc\n";
          }
      }
  ...
  // SPUPDATE 3 print final map SPUPDATE 5 commend out
  // Function *mainFunc = M.getFunction("main");
  // if (mainFunc) {
  //     // Check if main function has a return instruction. 
  //     // If it does, we'll insert printMap just before that.
  //     for (auto &BB : *mainFunc) {
  //         if (ReturnInst *returnInst = dyn_cast<ReturnInst>(BB.getTerminator())) {
  //             // Found return instruction, position the IRBuilder before this instruction
  //             IRBuilder<> endOfMainIRB(returnInst);
              
  //             FunctionCallee printMapCallee = M.getOrInsertFunction(
  //                 "printMap", FunctionType::getVoidTy(C));  // Assuming the function type matches
              
  //             // Call printMap
  //             endOfMainIRB.CreateCall(printMapCallee);
  //             break;
  //         }
  //     }
  // }
  ...
  if (!be_quiet) {
    ...
    if (!inst_blocks)
      ...
      OKF("Instrumented %u locations (%s mode, ratio %u%%). (%u free locations)", inst_blocks,
          getenv("AFL_HARDEN")
              ? "hardened"
              : ((getenv("AFL_USE_ASAN") || getenv("AFL_USE_MSAN"))
                     ? "ASAN/MSAN"
                     : "non-hardened"),
          inst_ratio, inst_free_blocks);
    OKF("AFLChurn instrumentation ratio %u%%", bb_select_ratio);
    if (inst_ages)
      module_ave_ages = module_total_ages / inst_ages;
    if (inst_changes)
      module_ave_chanegs = module_total_changes / inst_changes;
    if (inst_fitness)
      module_ave_fitness = module_total_fitness / inst_fitness;
    ...
    if (use_cmd_age && !is_one_commit) {
      ...
      OKF("Using Rank. Counted %u BBs with the average age of f(rank)=%.6f "
          "commits.",
          inst_ages, module_ave_ages);
    }
    if (use_cmd_change && !is_one_commit) {
      OKF("Using Change. Counted %u BBs with the average churn of "
          "f(changes)=%.6f churns.",
          inst_changes, module_ave_chanegs);
    }

    OKF("BB Churn Raw Fitness. Instrumented %u BBs with average raw fitness of "
        "%.6f",
